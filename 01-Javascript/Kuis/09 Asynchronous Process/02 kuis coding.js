...
async function getTollAccess() {
    try {
        const tollCard = await buyTollRoadCard(25);
        const topupTollCard = await topUpBalance(tollCard, 10);
        tollCard.balance = topupTollCard.balance;
        await useTollRoad(tollCard);
    } catch (err) {
